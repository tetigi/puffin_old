...
prop_vectorNormalize :: Vector Double -> Property
prop_vectorNormalize v =
  not (isZero v) ==> vectorSize (vectorNormalize v) ~= 1

prop_negVector :: Vector Int -> Bool
prop_negVector v@(Vector vs) = 
  negVector (negVector v) == v 

test_Vector = [ ("vectorDot", quickCheck prop_vectorDot)
              , ("vectorAppend", quickCheck prop_vectorAppend)
              , ("vectorNormalize", quickCheck prop_vectorNormalize)
              , ("negVector", quickCheck prop_negVector)
              ]
-- --------------------------
-- Matrix
